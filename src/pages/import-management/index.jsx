...
/* ---------- helpers ---------- */
const fmtMoneyUSD = (n, min = 0, max = 0) =>
      ...
      <div className="bg-card w-full max-w-3xl rounded-lg shadow-modal border border-border overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Info label="CIF Cost (USD)" value={fmtMoneyUSD(shipment.cifUsd, 0, 0)} />
          </div>
            ...
            {shipment.items.length === 0 ? (
              ...
              <div className="space-y-3">
                {shipment.items.map((it, idx) => (
                  <div
                    key={`${it.ociNumber}-${it.presentationCode}-${it.lotNumber}-${idx}`}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="text-sm">
                        ...
                        <div className="text-muted-foreground">
                          Lot: {it.lotNumber || "—"} · OCI: {it.ociNumber || "—"}
                        </div>
                      </div>
                      <div className="text-sm text-right">
                        <div>
                          Qty: <span className="font-medium">{toNumber(it.qty)}</span>
                        </div>
                        {/* ⬇️ unit con mínimo 2 decimales */}
                        <div>Unit: {fmtMoneyUSD(it.unitPrice, 2, 2)}</div>
                      </div>
                    </div>
                    <div className="mt-2">
...
export default function ImportManagementPage() {
  ...
  const shipments = useMemo(() => {
    ...
    for (const r of importRows || []) {
      ...
      if (!groups.has(sid)) {
        groups.set(sid, {
          ...
          arrivalDate: r.eta || "",
          cifUsd: Number(r.totalCostUsd || 0), // base opcional desde imports
          ociNumbers: new Set(),
          items: [],
        });
      }
      const g = groups.get(sid);
      if (r.ociNumber) g.ociNumbers.add(r.ociNumber);

      // prioriza "transit" si hay mezcla
    ...
    // 2) anexar ítems por OCI y sumar CIF en USD
    const list = Array.from(groups.values());
    for (const it of itemRows || []) {
      for (const g of list) {
        if (g.ociNumbers.has(it.ociNumber)) {
          g.items.push(it);
          if (!it.currency || it.currency === "USD") {
            g.cifUsd += toNumber(it.qty) * toNumber(it.unitPrice);
          }
        }
      }
    }
    ...
    arr.sort(
      (a, b) =>
        new Date(b.arrivalDate || 0).getTime() - new Date(a.arrivalDate || 0).getTime()
    );
    return arr;
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* ⬇️ Header fijo de tu app */}
      <Header />

      {/* ⬇️ Empuja el contenido hacia abajo para no tapar la barra superior */}
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="default" iconName="RefreshCcw">
                Refresh Data
              </Button>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {loading && (
                    <tr>
                      <td colSpan={6} className="px-6 py-6 text-sm">
                        Loading…
                      </td>
                    </tr>
                  )}
                    ...
                    shipments.map((s) => (
                      <tr key={s.shipmentId}>
                        ...
                        <td className="px-6 py-4">
                          <StatusPill value={s.importStatus} />
                        </td>
                        <td className="px-6 py-4 font-medium text-foreground">
                          {fmtMoneyUSD(s.cifUsd, 0, 0)}
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <Button variant="ghost" size="sm" iconName="Eye" onClick={() => openDetails(s)}>
                              View Details
                            </Button>
                          </div>
                        </td>
