...
const AnalyticsPanel = ({ currentLanguage, rows }) => {
  ...
  const quickStats = useMemo(() => {
    ...
    rows?.forEach((r) => {
      uniqueProducts.add(r.presentationCode);
      // Calcula daysSupply por fila
      ...
      if (daysSupply < 2) critical += 1;
      else if (daysSupply >= 2 && daysSupply <= 5) low += 1;
      else if (daysSupply > 5 && daysSupply <= 10) medium += 1;
      else if (daysSupply > 10) high += 1;
    });

    return {
      activeProducts: uniqueProducts.size,
      criticalStock: critical,
      airShipment: critical + low, // sugerimos envío aéreo para críticos + bajos
      healthyStock: high,
    };
  }, [rows]);
  ...
  const handleApplyFilters = () => {
    ...
    // En una versión futura aquí se aplicaría la lógica de filtrado
  };
      ...
      <div className="px-6 pb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
            <div className="text-2xl font-bold text-blue-800">{quickStats.activeProducts}</div>
            <div className="text-sm text-blue-600">
              {currentLanguage === 'es' ? 'Productos Activos' : 'Active Products'}
            </div>
          </div>
          <div className="text-center p-4 bg-red-50 rounded-lg border border-red-200">
            <div className="text-2xl font-bold text-red-800">{quickStats.criticalStock}</div>
            <div className="text-sm text-red-600">
              {currentLanguage === 'es' ? 'Stock Crítico' : 'Critical Stock'}
            </div>
          </div>
          <div className="text-center p-4 bg-amber-50 rounded-lg border border-amber-200">
            <div className="text-2xl font-bold text-amber-800">{quickStats.airShipment}</div>
            <div className="text-sm text-amber-600">
              {currentLanguage === 'es' ? 'Envío Aéreo' : 'Air Shipment'}
            </div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
            <div className="text-2xl font-bold text-green-800">{quickStats.healthyStock}</div>
